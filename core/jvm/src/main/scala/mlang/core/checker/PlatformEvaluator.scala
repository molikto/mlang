        ...
        case Abstract.Reference(up, index, closed) =>
          if (up > depth) {
            if (up == depth + 1 && recursivelyDefining.contains(index)) {
              ...
              s"${tunnel(evalOpenReference(up - depth - 1, index))}.deref()"
            }
          } else {
            if (closed == 1) {
              // first case is for non-guarded (inductive), and r.value actually is null, it will be filled latter
              // second is for guarded
              s"RecursiveReference(r${depth - up}($index)).deref()"
            } else if (closed == 0) {
              s"Reference(r${depth - up}($index)).deref()"
            } else {
              s"r${depth - up}($index)" // formal parameter generated by the same eval code
            }
          }

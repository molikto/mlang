...
object EType {
  case class Function(val implicitt: Boolean) extends EType
  object Function {
    val Implicit = Function(true)
    val NonImplicit = Function(false)
    def get(i: Boolean): Function = if (i) Implicit else NonImplicit
  }
  case class Record(val names: Seq[Name], implicits: Seq[Boolean]) extends EType
  case class Sum(val names: Seq[Name], implicits: Seq[Seq[Boolean]]) extends EType
}